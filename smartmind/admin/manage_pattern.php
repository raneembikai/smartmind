...
// Fetch all pattern questions
$query = "SELECT * FROM pattern_questions ORDER BY ID";
...
<head>
    <meta charset="UTF-8">
    <title>Manage Pattern Questions</title>
...
<body class="bg-black text-white font-sans">
    ...
    <div class="max-w-6xl mx-auto p-6 mt-10 bg-gray-900 rounded-xl shadow-2xl">
        <h2 class="text-3xl font-bold text-center text-blue-400 mb-6">Manage Pattern Questions</h2>

        <div class="flex justify-between mb-6">
            <a href="add_pattern.php" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg font-semibold transition duration-300">
                ➕ Add New Question
            </a>
                    ...
                    <tr class="hover:bg-gray-700 transition">
                        ...
                        <td class="py-3 px-4">
                            <?php if (!empty($row['question_image'])): ?>
                                <img src="pattern_images/<?= $row['question_image'] ?>" alt="Pattern" class="w-20 h-auto mx-auto rounded">
                            <?php else: ?>
                        ...
                        <td class="py-3 px-4 space-x-2">
                            <a href="edit_pattern.php?id=<?= $row['id'] ?>" class="bg-green-600 hover:bg-green-700 px-3 py-1 rounded-md">Edit</a>
                            <a href="delete_pattern.php?id=<?= $row['id'] ?>" onclick="return confirm('Are you sure?')" class="bg-red-600 hover:bg-red-700 px-3 py-1 rounded-md">Delete</a>
                        </td>
                    </tr>
                    <?php } ?>
                    <?php if (mysqli_num_rows($result) == 0): ?>
                    <tr>
                        <td colspan="9" class="py-6 text-center text-gray-400">No pattern questions found.</td>
                    </tr>
                    <?php endif; ?>
                </tbody>
            </table>
